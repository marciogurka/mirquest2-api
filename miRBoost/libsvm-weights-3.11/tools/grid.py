...
if not is_win32:
       svmtrain_exe = "libsvm-weights-3.11/svm-train"
       gnuplot_exe = "/software/bin/gnuplot"
else:
       # example for windows
...
global out_filename, png_filename, weight_filename
...
def process_options(argv=sys.argv):
    ...
    global dataset_pathname, dataset_title, pass_through_string
    global svmtrain_exe, gnuplot_exe, gnuplot, out_filename, png_filename, weight_filename
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x {0} "cd {1}; {2} -c {3} -g {4} -v {5} -W {6} {7} {8}"'.format \
          (self.host,self.cwd, \
           svmtrain_exe,c,g,fold,weight_filename,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
            if str(line).find("Cross") != -1:
                return float(line.split()[-1][0:-1])
...
class TelnetWorker(Worker):
    ...
    def run(self):
        ...
        tn.write("exit\n")               
    def run_one(self,c,g):
        cmdline = '{0} -c {1} -g {2} -v {3} -W {4} {5} {6}'.format \
          (svmtrain_exe,c,g,fold,weight_filename,pass_through_string,dataset_pathname)
